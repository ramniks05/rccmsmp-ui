...
export class AdministrativeUnitsComponent implements OnInit {
  ...
  deleteUnit(unit: any): void {
    if (confirm(`Are you sure you want to delete "${unit.unitName}"?`)) {
      this.isLoading = true;
      this.adminService
        .deleteAdminUnit(unit.unitId)
        .pipe(
          catchError((error) => {
            this.isLoading = false;
            this.showError(
              error.error?.message || 'Failed to delete administrative unit',
            );
            return throwError(() => error);
          }),
        )
  ...
  private handleError(error: any): void {
    if (error.error?.message) {
      ...
      this.errorMessage =
        'Failed to load administrative units. Please try again.';
    }
  }
...
@Component({
  ...
  template: `
    <h2 mat-dialog-title>
      {{ data.mode === 'create' ? 'Create' : 'Edit' }} Administrative Unit
    </h2>
    <mat-dialog-content>
      <form [formGroup]="unitForm" class="unit-form">
        <mat-form-field appearance="outline" class="full-width">
          ...
          />
          <mat-error *ngIf="unitForm.get('unitCode')?.hasError('required')"
            >Unit code is required</mat-error
          >
        </mat-form-field>
        ...
        <mat-form-field appearance="outline" class="full-width">
          ...
          />
          <mat-error *ngIf="unitForm.get('unitName')?.hasError('required')"
            >Unit name is required</mat-error
          >
        </mat-form-field>
        ...
        <mat-form-field appearance="outline" class="full-width">
          ...
          <mat-error *ngIf="unitForm.get('unitLevel')?.hasError('required')"
            >Unit level is required</mat-error
          >
        </mat-form-field>
        ...
        <mat-form-field appearance="outline" class="full-width">
          ...
          <input
            matInput
            type="number"
            formControlName="lgdCode"
            placeholder="Enter LGD code"
          />
          <mat-error *ngIf="unitForm.get('lgdCode')?.hasError('required')"
            >LGD code is required</mat-error
          >
        </mat-form-field>
        ...
        <mat-form-field
          appearance="outline"
          class="full-width"
          *ngIf="unitForm.get('unitLevel')?.value !== 'STATE'"
        >
          <mat-label>Parent Unit</mat-label>
          <mat-select formControlName="parentUnitId">
            <mat-option [value]="null">None</mat-option>
            <mat-option
              *ngFor="let parent of filteredParentUnits"
              [value]="parent.unitId"
            >
...
export class AdminUnitDialogComponent {
  ...
  filterParentUnits(level: string): void {
    ...
    this.filteredParentUnits = (this.data.parentUnits || []).filter(
      (unit: any) => allowedLevels.includes(unit.unitLevel),
    );
  }
  ...
  onSave(): void {
    if (this.unitForm.valid) {
      ...
      if (this.data.mode === 'create') {
        ...
        this.adminService
          .updateAdminUnit(this.data.unit.unitId, formValue)
          .pipe(
            catchError((error) => {
              this.isLoading = false;
              this.snackBar.open(
